...
class SimpleHTTPRequestHandler(http.server.BaseHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        f.write(("<style>span.filesize{text-align-last:right;} span.timestamp{color:gray;margin-right:300px;margin-left:300px;} div{display: flex;justify-content: space-between;align-items:center;}</style>\n").encode())
        ...
        for name in list:
            ...
            f.write(('<li><div><a href="%s"><span class="filName">%s</span></a><div><span class="filesize">%s</span><span class="timestamp">%s</span></div></div></li>\n'
                    % (urllib.parse.quote(linkname), html.escape(displayname), html.escape(filesize_str), html.escape(mtime_str))).encode())
        f.write(b"</ul>\n<hr>\n</body>\n</html>\n")
        length = f.tell()
        f.seek(0)
        self.send_response(200)
